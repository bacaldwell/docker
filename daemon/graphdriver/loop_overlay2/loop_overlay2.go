...
// TODO: refcount loopback image mounts
...
func Init(home string, options []string, uidMaps, gidMaps []idtools.IDMap) (graphdriver.Driver, error) {
	...
	if len(storeImgDirs) == 0 ||
		...
		return nil, fmt.Errorf("loop_overlay2: neither loopback_root or loopback_fallback are writable. No usable path for image metadata")
	}
...
func useNaiveDiff(home string) bool {
	useNaiveDiffLock.Do(func() {
		if err := hasOpaqueCopyUpBug(home); err != nil {
			logrus.Warnf("Not using native diff for loop_overlay2: %v", err)
	...
	if _, err := os.Lstat(dir); err != nil {
		...
		if err == nil {
			...
			if err = d.createParent(parent, opts); err != nil {
				return fmt.Errorf("loop_overlay2: unable to recursively recreate layer %s: %v\n", parent, err)
			}
...
func (d *Driver) findNewLoopbackPath(id string) (string, error) {
	...
	// Return empty string
	err = fmt.Errorf("loop-overlay2: could not find a writeable path for loopback device storage")
	return retPath, err
}
...
func (d *Driver) unmountAndDetachLoopIfNeeded(id string, loopFile string) (bool, error) {
	...
	for _, e := range entries {
		if e.Mountpoint == loopFile {
			...
			if loopDev == nil {
				defer f.Close()
				return didUnmount, fmt.Errorf("loop_overlay2: couldn't get loopback device for mount %s",
							      loopFile)
			} else {
				logrus.Infof("Found loop device %s attached to file %", loopDev.Name(),
					     loopFile)
				loopDev.Close()
			}
...
func (d *Driver) findLoopAttached(loopFile string) (*os.File, error) {
	// Try opening loopFile
	f, err := os.Open(loopFile)
	if err != nil {
		return nil, err
	}

	// TODO: we need a call in loopback library that doesn't log so many ERR messages
	loopDev := loopback.FindLoopDeviceFor(f)
...
func (d *Driver) mountStores(paths []string) ([]string, error) {
	if paths == nil {
		return nil, fmt.Errorf("loop_overlay2: no paths of lower layers to mount")
	}
	...
	for _, p := range paths {
		...
		if err != nil {
			...
			if err == nil {
				...
				logrus.Debugf("loop_overlay2: read-write attached loop dev %s to path %s", loopDev.Name(), loopFile)
				...
				logrus.Debugf("loop_overlay2: read-only attached loop dev %s to path %s", loopDev.Name(), loopFile)
			} else {
...
func (d *Driver) Exists(id string) bool {
	...
	if !isMounted {
		...
		logrus.Errorf("loop_overlay2: Exists called on id %s, which is present, but loop file %s not mounted", id, loopFile)
		return false
	} else {
		return true
	}
...
func (d *Driver) createFilesystem(devname string) (err error) {
	...
	defer func() {
		if err != nil {
			...
			logrus.Debugf("loop_overlay2: Successfully created filesystem %s on device %s", d.filesystem, devname)
		}
	}()
...
func (d *Driver) createLoopbackImg(path string, size int64) (*os.File, error) {
	...
	if err := d.ensureImage(path, size); err != nil {
		logrus.Debugf("loop_overlay2: Error device ensureImage (%s): %s", path, err)
		return nil, err
	}
...
func (d *Driver) mountLoopbackDevice(dev, path, mountLabel string, mountRW bool) error {
	...
	logrus.Debugf("loop_overlay2: Mounted loopback device %s at %s, RW=%t", dev, path, mountRW)
...
func (d *Driver) unmountLoopbackDevice(mountPath string) (bool, error) {
	...
	if count := d.ctr.Decrement(mountPath); count > 0 {
		return false, fmt.Errorf("loop_overlay2: do not have exclusive access on unmount of %s", mountPath)
	}
...
func (d *Driver) collectRepositories(lowerDir string, upperDir string) ([]byte, error) {
	var lowerStore = store{}
	var upperStore = store{}

	if lowerDir != "" {
		lowerFile, err := os.Open(path.Join(lowerDir, "repositories.json"))
		if err != nil {
			return nil, err
		}
		...
		if err := json.NewDecoder(lowerFile).Decode(&lowerStore); err != nil {
			return nil, err
		}
	}

	if upperDir != "" {
		upperFile, err := os.Open(path.Join(upperDir, "repositories.json"))
		if err != nil {
			return nil, err
		}
		...
		if err := json.NewDecoder(upperFile).Decode(&upperStore); err != nil {
			return nil, err
		}
	}

	// Start with all entries from lowerStore and add entries
	// found in upperStore which do not exist in lowerStore
	for repoName, upperRepository := range upperStore.Repositories {
		tempRepository, exists := lowerStore.Repositories[repoName]
		if !exists || tempRepository == nil {
			lowerStore.Repositories[repoName] = upperRepository
		}
	}

	jsonData, err := json.Marshal(lowerStore)
	if err != nil {
		return nil, err
	}
...
func (d *Driver) writeRepository(jsonData []byte, mountPoint string) error {
	outFile := path.Join(mountPoint, "repositories.json")

	return ioutils.AtomicWriteFile(outFile, jsonData, 0600)
}
...
func (d *Driver) mergeStores(loopStoreImgDirs []string) error {
	...
	if loopStoreImgDirs == nil {
		return fmt.Errorf("loop_overlay2: no IDs of lower store paths to mount")
	}
	...
	if len(loopMounts) > 1 {
		...
		mergedRepos, err := d.collectRepositories(lowerDir, upperDir)
		if err != nil {
			logrus.Error("loop_overlay2: error collecting data from either repositories.json")
			return err
		}
		...
		if err != nil {
			logrus.Errorf("loop_overlay2: error creating overlay mount to %s: %v", mountTarget, err)
			return err
		}

		if err := d.writeRepository(mergedRepos, mountTarget); err != nil {
			logrus.Error("loop_overlay2: error writing out the combined repositories.json")
			return err
		}
		...
		if err != nil {
			logrus.Errorf("loop_overlay2: error creating bind mount from %s to %s: %v", lowers[0], mountTarget, err)
			return err
		}
	} else {
...
func (d *Driver) createStores(storeImgDir string) error {
	...
	// Create stores in mounted loopback filesystem
	fms, err := layer.NewFSMetadataStore(path.Join(loopMountDiff, "layerdb"))
	if err != nil {
		return err
	}
	...
	ifs, err := image.NewFSStoreBackend(path.Join(loopMountDiff, "imagedb"))
	if err != nil {
		return err
	}

	_, err = image.NewImageStore(ifs, layerStore)
	if err != nil {
		return err
	}
	...
	if !didUnmount {
		return fmt.Errorf("loop_overlay2: failed to unmount %s", loopMount)
	}
