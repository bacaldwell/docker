...
func (d *Driver) Exists(id string) bool {
	...
	// TODO what if dir is present and id is not Rw layer, but
	// loopback device is not mounted. Does Exists() get called
	// this situation?

	if isMounted, _ := mount.Mounted(d.dir(id)); !isMounted {
		return false
	} else if isMounted {
		return true
	}
...
func (d *Driver) mergeStores(loopStoreImgDirs []string) error {
	...
	defer func() {
		if err != nil {
			// TODO: why does this never get run when there is a problem with overlay
			// mount
			logrus.Debug("loop_overlay2: cleaning up loopback mounts")
	...
	if len(loopMounts) > 1 {
		...
		err = syscall.Mount(lowers[0], mountTarget, "bind", syscall.MS_BIND, "")
		if err != nil {
...
func (d *Driver) createStores(storeImgDir string) error {
	...
	defer func() {
		...
		if err != nil {
			if isMounted, err2 := mount.Mounted(loopMount); err2 == nil && isMounted {
				if err := syscall.Unmount(loopMount, syscall.MNT_DETACH); err != nil {
					logrus.Errorf("loop_overlay2: failed to unmount %s", loopMount)
				}
			}

			os.RemoveAll(loopMount)
			os.Remove(storeImg)
		}
	}()
