...
func ProbeFsType(device string) (string, error) {
	...
	if uint64(l) != maxLen {
		return "", fmt.Errorf("loop_overlay2: unable to detect filesystem type of %s, short read", device)
	}
	...
	return "", fmt.Errorf("loop_overlay2: Unknown filesystem type on %s", device)
}
