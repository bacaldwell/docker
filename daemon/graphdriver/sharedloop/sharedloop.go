...
func collectRepositories(lowerDir string, upperDir string) ([]byte, error) {
	var lowerStore = store{}
	var upperStore = store{}
	var decodedStores []string
	...
	if lowerDir != "" {
		...
		if err == nil {
			...
			decodedStores = append(decodedStores, "lower")
		}
	...
	if upperDir != "" {
		...
		if err == nil {
			...
			decodedStores = append(decodedStores, "upper")
		}
	}

	// determine which stores too merge
	if decodedStores == nil {
		return nil, fmt.Errorf("%s: error collecting data from either repositories.json", driverName)
	} else if len(decodedStores) < 2 {
		if stringInSlice("upper", decodedStores) {
			lowerStore = upperStore
		} else if !stringInSlice("lower", decodedStores) {
			// This should never happen
			return nil, fmt.Errorf("%s: unexpected error", driverName)
		}
		// else: only lower store will be used
...
func writeRepository(jsonData []byte, mountPoint string) error {
...
func (d *Driver) mergeStores(loopStoreImgDirs []string) error {
	...
	if len(loopMounts) > 1 {
		...
		mergedRepos, err := collectRepositories(lowerDir, upperDir)
		if err != nil {
			logrus.Errorf("%v", err)
			mergedRepos = nil
		}
		...
		if mergedRepos != nil {
			if err := writeRepository(mergedRepos, mountTarget); err != nil {
				logrus.Error("%s: error writing out the combined repositories.json", driverName)
				return err
			}
		}
		// else: new repositories.json will be written
